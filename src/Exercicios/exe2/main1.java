...
public class main1 {
    ...
    public static void apenasNumeros(String entrada, boolean valido_invalido, boolean allNumbers) {
        ...
        if (valido_invalido && allNumbers) {
            ...
            System.out.println(tipo.formataString(entrada));
        return;
    }
    ...
    public static String verificandoFormato(String entrada, int qtd_numeros) {
        ...
        switch (qtd_numeros) {
            case 8:
                String verifica_fixo_sem_ddd = (entrada_lista[4] == '-') ? "[Formatado Corretamente] Fixo sem DDD - " + entrada : formatacoes.FIXO_SEM_DDD.formataString(cortandoCaracteres(entrada));
                return verifica_fixo_sem_ddd;
            case 9:
                String verifica_celular_sem_ddd = (entrada_lista[5] == '-') ? "[Formatado Corretamente] Celular sem DDD - " + entrada : formatacoes.CELULAR_SEM_DDD.formataString(cortandoCaracteres(entrada));
                return verifica_celular_sem_ddd;
            case 10:
                String verifica_fixo_com_ddd = ((entrada_lista[0] == '(') &&
                                                (entrada_lista[3] == ')') &&
                                                (entrada_lista[8])== '-') ? "[Formatado Corretamente] Fixo com DDD - " + entrada : formatacoes.FIXO_COM_DDD.formataString(cortandoCaracteres(entrada));
                return verifica_fixo_com_ddd;
            case 11:
                String verifica_celular_com_ddd = ((entrada_lista[0] == '(') &&
                                                (entrada_lista[3] == ')') &&
                                                (entrada_lista[9])== '-') ? "[Formatado Corretamente] Celular com DDD - " + entrada : formatacoes.CELULAR_COM_DDD.formataString(cortandoCaracteres(entrada));
